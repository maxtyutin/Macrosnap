import urllib.request
import json
import base64
import os

part_a = "AIzaSyBrjH5Jtqm98P"
part_b = "dV3431eLY6caxHXFG_Nd0"
gemini_key = part_a + part_b

# Find an existing image to test with
test_img_dir = "images/uploads"
test_img = None
if os.path.exists(test_img_dir):
    files = os.listdir(test_img_dir)
    if files:
        test_img = os.path.join(test_img_dir, files[0])
        print(f"Using test image: {test_img}")

if not test_img:
    print("No test image found, creating a small dummy JPEG...")
    # Create minimal 1x1 JPEG
    import struct
    tiny_jpg = bytes([
        
    ])
    test_img = "/tmp/test_bot_image.jpg"
    with open(test_img, "wb") as f:
        f.write(tiny_jpg)

with open(test_img, "rb") as f:
    img_data = f.read()

base64_data = base64.b64encode(img_data).decode('utf-8')

prompt_text = "What food do you see? Reply with just a JSON object: {\"name\": \"food name\", \"ingredients\": []}"

models = ["gemini-2.0-flash", "gemini-2.5-flash", "gemini-3.5-flash"]

for model_name in models:
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={gemini_key}"
    print(f"\nTesting {model_name} with image...")

    payload = {
        "contents": [{
            "parts": [
                {"text": prompt_text},
                {"inlineData": {"mimeType": "image/jpeg", "data": base64_data}}
            ]
        }]
    }

    req = urllib.request.Request(
        url,
        data=json.dumps(payload).encode('utf-8'),
        headers={'Content-Type': 'application/json'}
    )

    try:
        with urllib.request.urlopen(req, timeout=20) as response:
            res_data = json.loads(response.read().decode('utf-8'))
            text = res_data['candidates'][0]['content']['parts'][0]['text']
            print(f"SUCCESS {model_name}: {text[:200]}")
    except Exception as e:
        print(f"ERROR {model_name}: {e}")
        if hasattr(e, 'read'):
            try:
                print(e.read().decode('utf-8')[:300])
            except:
                pass
