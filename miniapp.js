// Telegram Mini App Client JS
let tg = window.Telegram ? window.Telegram.WebApp : null;
let userId = "tg_demo_user";
...
function loadApp() {
  ...
  if (tg) {
    ...
    document.documentElement.style.setProperty('--tg-btn', tg.themeParams.button_color || '#007aff');
  ...
  fetch(`/api/sync?userId=${userId}`)
    ...
    .then(data => {
      if (data && data.success && data.user) {
        ...
        currentUser = {
          email: userId,
          name: tg && tg.initDataUnsafe.user ? tg.initDataUnsafe.user.first_name : "Пользователь",
          gender: "male",
          age: 25,
          height: 175,
          weight: 70,
      ...
      if (tg && tg.initDataUnsafe.user) {
        document.getElementById('greeting-text').innerText = `Привет, ${tg.initDataUnsafe.user.first_name}! 👋`;
      }
